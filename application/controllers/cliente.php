...
class Cliente extends AbstractAccess {
	...
	public function add()
	{
		...
		$this->form_validation->set_rules('no_interior', 'No. Interior', 'trim|required|strtolower|xss_clean');
		$this->form_validation->set_rules('colonia', 'Colonia', 'trim|strtolower|ucwords|max_length[20]|xss_clean');
		$this->form_validation->set_rules('codigo_postal', 'Código Postal', 'trim|required|max_length[7]|xss_clean');
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->_vista('form-nuevo-cliente', 'clientes');
		} else {
