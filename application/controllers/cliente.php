...
class Cliente extends AbstractAccess {
	...
	public function add($tipo = null)
	{
		...
		// SI ES CLIENTE NORMAL AGRUEGO MAS REGLAS DE VALIDACION
		if ($tipo == 'normal') {
			//Datos basicos
		...
		if ($this->form_validation->run() === FALSE) {
			...
			if ($tipo == 'normal')
			{
				...
				if($this->clienteModel->insert($basica_cliente))
				{
					...
					$respuesta = array(
						'exito' => ($bol_contactos and $bol_sistemas and $bol_equipos),
	 ...
	 * @author Diego Rodriguez | Luis Macias
	 **/
	public function _add($accion = null)
	{
		switch ($accion) {
			case 'addRapido':
				...
				if ($this->form_validation->run() === FALSE) {
					...
					if($this->clienteModel->insert($basica_cliente))
					{
						...
						echo json_encode(array('exito' => ($bol_contactos), 'razon_social' => $basica_cliente->razon_social));
					} else
					{
						header('Content-Type: application/json');
						echo json_encode(array('exito' => FALSE, 'msg' => validation_errors()));
